...
import { indexEntities, ROOT_POINTER } from "../../document";
...
export function JsonTree({ editor }: { editor: JsonEditorStore }) {
  ...
  // Built once here (memoized on `root`) and threaded down, so each row resolves a
  // reference with an O(1) lookup instead of re-walking the tree.
