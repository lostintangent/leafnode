...
export function TreeNode({
  ...
  const background = view.flashing
    ? `color-mix(in srgb, ${theme.theme.accent} 22%, transparent)`
    : view.change === "added"
      ? theme.diff.added
      : view.change === "changed"
        ? theme.diff.changed
  ...
  return (
    <div>
      <div
        ref={rowRef}
        className={cx(
          "group/row relative flex min-w-0 items-center gap-1 py-0.5 pr-2 font-mono text-[13px] leading-6 transition-colors hover:bg-hover",
      ...
      >
        {(view.dropBefore || view.dropAfter) && (
          <span
            ...
            style={{ left: indent, background: theme.theme.accent }}
          />
        )}
        ...
        {draggable && (
          <span
            ...
            className="absolute inset-y-0 flex w-4 -translate-x-full cursor-grab items-center justify-center text-muted opacity-0 transition-opacity group-hover/row:opacity-100 active:cursor-grabbing [@media(hover:none)]:hidden"
            style={{ left: indent }}
        ...
        {edge.kind === "member" &&
          (view.editingKey ? (
            ...
            <span
              className={cx("shrink-0 text-text", !view.readOnly && !view.busy && "cursor-text")}
        ...
        {node.kind !== "scalar" ? (
          ...
          <Checkbox
            ...
            onChange={(checked) => actions.replaceValue(node.id, checked)}
          ...
          <span
            className={cx("min-w-0 truncate", !view.readOnly && !view.busy && "cursor-text")}
        ...
        {view.busy ? (
          ...
          >
            <Loader2 className="size-3.5 animate-spin" style={{ color: theme.theme.accent }} />
          </span>
        ) : (
